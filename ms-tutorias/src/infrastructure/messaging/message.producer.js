...
const connect = async () => {
  try {
    ...
    console.log(
      "[MS_Tutorias] Conectado a RabbitMQ y exchange de tracking asegurado."
    );
  } catch (error) {
    console.error(
      "[MS_Tutorias] Error al conectar con RabbitMQ:",
      error.message
    );
    setTimeout(connect, 5000);
  }
...
// Iniciamos la conexión al cargar el archivo (o podrías llamarlo desde app.js)
connect();
...
const publishTrackingEvent = async (payload) => {
  ...
  try {
    ...
    console.error(
      `[MS_Tutorias] Error al publicar evento de tracking:`,
      error.message
    );
  }
};
