...
const findBloqueosByTutor = async (idTutor) => {
  ...
  const queryText =
    "SELECT idbloqueo, idtutor, fechainicio, duracionminutos, idestudiante FROM bloqueos WHERE idtutor = $1 ORDER BY fechainicio ASC";
  try {
...
const saveBloqueo = async (datosBloqueo) => {
  ...
  const queryText =
    "INSERT INTO bloqueos(idtutor, fechainicio, duracionminutos, idestudiante) VALUES($1, $2, $3, $4) RETURNING *"; // RETURNING * devuelve la fila insertada
  try {
